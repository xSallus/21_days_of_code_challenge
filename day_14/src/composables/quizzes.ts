...
export let isLoadingQuizzes:Ref<boolean> = ref(true)
export let hasFinishedQuiz:Ref<boolean> = ref(false)
export let isSubmittingQuiz:Ref<boolean> = ref(false)
	  ...
	  .get<QuizRepsponse[]>('/questions?limit=10')
		.then(({ data }) => {
			quizzes = data.map((item) => {
				const points = item.difficulty === 'easy' ? 50 : (item.difficulty === 'medium' ? 100 : 250)
